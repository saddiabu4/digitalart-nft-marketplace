      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-12 mb-20">
          <div className="lg:col-span-2 space-y-8">
            <button onClick={() => setPage('home')} className="text-2xl font-bold tracking-tighter">
              Digital<span className="lime-accent">Art</span>
            </button>
            <p className="text-white/40 leading-relaxed max-w-sm">
              Discover NFTs by category, track the latest drop, and follow the collections you love. Enjoy the world of digital creation!
            </p>
            <div className="flex gap-4">
              {[Facebook, Send, Twitter, Linkedin].map((Icon, idx) => (
                <a key={idx} href="#" className="w-10 h-10 rounded-full glass-card border-none flex items-center justify-center hover:bg-lime-accent hover:text-black transition-all">
                  <Icon size={18} />
                </a>
              ))}
            </div>
        ...
        <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-white/30 text-sm">
            © {new Date().getFullYear()} DigitalArt - by davixq
          </p>
